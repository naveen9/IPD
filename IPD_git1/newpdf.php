...
include("hede.php");
$name=$_SESSION['uname'];
$bill_id_opd=$_SESSION['bill_id_opd'];
$billeddate=$_SESSION['billeddate'];
//$name=rani;
$v_id=$_SESSION['v_id'];
//$v_id=215;
...
$pdf->ln(-.3);
$pdf->SetFont('Arial','B',10);
$pdf->cell(0, 1.0, "Bill Id:    $bill_id_opd", 0, 1, 'L');
$pdf->cell(3.9, -1, "Bill Date:    $billeddate", 0, 1, 'R');
$pdf->cell(6.8, 1.0, "Receptionist:  $name", 0, 1, 'R');
//$pdf->SetFont('Arial','',12);
...
//$pdf->ln(.6);
//$pdf->SetFont('Arial','',12);
//$pdf->cell(6.8, -.7, "Receptionist:  $reciept_id", 0, 0, 'R');
...
$select21=("select * from opd_items where bill_id='$bill_id_opd'");
$result = mysql_query($select21) or die( "Could not execute opd line no. 160: {$sql}");
$w=2;
$h=.3;
$pdf->ln(1);
