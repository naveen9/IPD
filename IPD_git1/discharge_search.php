        ...
        else
        {
            ...
            else
            {
                while($search_result=mysql_fetch_array($q))
                {
                    ...
                    echo $_SESSION['bed_no']=$search_result[6];
...
if($_POST['find1'])
{
    if($user=='user')
    {
        ...
        {
            $q=mysql_query("select P.patient_id,P.p_name,P.p_age,P.p_gender,P.p_mob,V.visit_id,V.bed_no from patient_info P,visit_register V where V.bed_no='$search1' and V.bed_no!=0 and V.p_id=P.patient_id group by P.patient_id");
            $found=mysql_num_rows($q);
            if($found==0)
            {
                header("location: discharge.php?error_msg=".urlencode("Patient Not Found! Please Add the Patient!"));
