...
header("location: opd-visit-summary.php?error_msg=".urlencode("Please Enter a Searchword!"));
}
else
{
$q=mysql_query("select P.patient_id,P.p_name,P.p_age,P.p_gender,P.p_mob,V.visit_id,V.bed_no
              from patient_info P,visit_register V where P.p_name='$search' or P.patient_id='$search' 
              and V.bed_no!='Discharge patient' and V.p_id=P.patient_id group by P.patient_id")or die(mysql_error());
...
if($found==0)
{
    ...
    unset($_SESSION['bed_no']);
    unset($_SESSION['crnt_gvn_anmt']);
header("location: opd-visit-summary.php?error_msg=".urlencode("Patient Not Found! Please Add the Patient!"));
}
else
{
...
header("location: opd-visit-summary.php?error_msg1=".urlencode("Patient Found"));
}
}
    }
   else
...
if($_POST['find1'])
{
    $search1=$_POST['search1'];
        ...
        {
            $q=mysql_query("select P.patient_id,P.p_name,P.p_age,P.p_gender,P.p_mob,V.visit_id,V.bed_no from patient_info P,visit_register V where V.bed_no='$search1' and V.bed_no!='Discharge patient' and P.patient_id=V.p_id");
            $found=mysql_num_rows($q);
            if($found==0)
            {
                unset($_SESSION['p_id']);
