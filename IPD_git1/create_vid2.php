...

{
    ...
    if($search1=="")
    {
        ...
        header("location: diagnostics.php?error_msg=".urlencode("Please Enter a Searchword!"));
    }
    else
    {
        ...
        if($found==0)
        {
            ...
            unset($_SESSION['bed_no']);
            unset($_SESSION['crnt_gvn_anmt']);
            header("location: diagnostics.php?error_msg=".urlencode("Patient Not Found! Please Add the Patient!"));
        }
        else
        {
            while($search_result=mysql_fetch_array($q))
            ...
            $_SESSION['b_id']=$fet_b_id[0];
            $b_id=$_SESSION['b_id'];//storing bill id in session
            header("location: diagnostics.php?error_msg1=".urlencode("Patient Found"));
        }
    }
