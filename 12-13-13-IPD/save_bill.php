...
echo "<br>bene54=".$bene=mysql_query("insert into beneficairy_payment values('','$v_id','$p_name','$b_id','$procedure','$proc_id','$doc_incharge','$total','$billdate','$deduction','$share','$tds','',$remark','','')");
...
else
{
   ...
   header('location:consultationg.php');
}
}
...
if($_POST['make_payment'])
{
    ...
    if(!empty($s_proc_id) or !empty($v_id) or !empty($p_id))
    {
        ...
        unset($_SESSION['grand_total']);
        unset($_SESSION['s_proc_id']);
        header("location:consultation.php?error_msg=".urlencode("Data are save"));
