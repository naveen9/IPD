...
<div id="opd_bill"><span style="margin-left:7px;">Doctor Collection</span></div>
	   ...
	   ?>
        ...
        <div class="todate_w f_l" style="margin-left:30px;">
		<input type="radio" name="datecheck" style="margin:6px 15px 0px 0px;" /></div> 
        ...
        <span style=" margin-left:0px;"><input type="text" name="name" placeholder="ID/Name/Ph/Email" id="inputString" onKeyUp="lookup(this.value);" onBlur="fill();" /></span>
...
<div align="left" class="suggestionList" id="autoSuggestionsList">
                ...
                <span><input type="submit" name="search" value="Search" class="btn"/></span>
				</form>
       </div>
	  ...
	  if($_POST['monthDate']&& $_POST['datecheck']){
	   ...
	   $month=$data[0];
	    ...
	    $query = "SELECT COUNT(*) as num FROM beneficairy_payment where doctor_incharge='$docName' && YEAR( DATE ) =  '$year'
AND MONTH( DATE ) =  '$month' && `check`=''";
    ...
    /* Get data. */
    $query = "SELECT * FROM beneficairy_payment where doctor_incharge='$docName' && YEAR( DATE ) =  '$year'
AND MONTH( DATE ) =  '$month'  && `check`='' ";
    $benQuery = mysql_query($query);
	include("paging/paging_demo.php");
  ...
  $total_pages = $total_pages['num'];
   ...
   /* Get data. */
   $query = "SELECT * FROM beneficairy_payment where `check`='' LIMIT $start,$limit";
   $benQuery = mysql_query($query);
	 include("paging/paging_demo.php");
	 }
      ...
      <div style=" width:100%;">
          ...
          <div class="l_ft" style="padding-top:4px; width:38px;">&nbsp;</div>
           ...
           <form method="post" action=""  id="percent" name="percent">               
      ...
      <div style="background:url(ccc.png); height:30px; padding-top:10px;">
		<div class="l_ft" style="padding-top:4px; width:38px;"><input type="checkbox" name="chk[]" value="<?php echo $benValues['bp_id'];?>" style="width:30px;" /></div>
             ...
             <div class="l_ft be_margin" id="fin_shar_<?php echo $i; ?>">0</div>
                ...
                <input type="hidden" id="fin_tds_hid_<?php echo $i; ?>" /> 
             </div>
             <div class="cls"></div>
