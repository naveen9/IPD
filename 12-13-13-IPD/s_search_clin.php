...
header("location: clienthome.php?error_msg=".urlencode("Please Enter a Searchword!"));
}
else
{
$q=mysql_query("select P.patient_id,P.p_name,P.p_age,P.p_gender,P.p_mob,V.visit_id,V.bed_no from patient_info P,visit_register V where P.p_name='$search' and V.bed_no!=0 and V.p_id=P.patient_id group by P.patient_id");
...
if($found==0)
{
    ...
    unset($_SESSION['bed_no']);
    unset($_SESSION['crnt_gvn_anmt']);
header("location: clienthome.php?error_msg=".urlencode("Patient Not Found! Please Add the Patient!"));
}
else
{
...
header("location: clienthome.php?error_msg1=".urlencode("Patient Found"));
}
}
    }
    else
        ...
        if($user=='admin')
    {
        ...
        {
            $q=mysql_query("select P.patient_id,P.p_name,P.p_age,P.p_gender,P.p_mob,V.visit_id,V.bed_no from patient_info P,visit_register V where P.p_name='$search' and V.p_id=P.patient_id group by P.patient_id");
            $found=mysql_num_rows($q);
            if($found==0)
            {
                unset($_SESSION['p_id']);
