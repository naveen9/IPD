 ...
 if(!empty($s_proc_id) or !empty($v_id) or !empty($p_id))
  {
     ...
     header("location:s_consultation.php?error_msg1=".urlencode("Saved bill"));
  }
else
{
   // echo naveen;
   ...
   header('location:s_consultationg.php');
}
}
...
if($_POST['make_payment'])
{
    ...
    if(!empty($s_proc_id) or !empty($v_id) or !empty($p_id))
    {
        ...
        header("location:s_consultation.php?error_msg=".urlencode("Saved data"));
