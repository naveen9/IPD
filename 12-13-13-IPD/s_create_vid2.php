...
if($_POST['create_visit_id'])
{
    if($p_id=="")
    {
        ...
        {
            header("location: s_diagnostics.php");
        }
        else
        {
...

{
    ...
    if($search1=="")
    {
        ...
        header("location: s_diagnostics.php?error_msg=".urlencode("Please Enter a Searchword!"));
    }
    else
    {
        $q=mysql_query("select P.patient_id,P.p_name,P.p_age,P.p_gender,P.p_mob,V.visit_id,V.bed_no from patient_info P,visit_register V where V.bed_no='$search1' and V.bed_no!=0 and P.patient_id=V.p_id");
        ...
        if($found==0)
        {
            ...
            unset($_SESSION['bed_no']);
            unset($_SESSION['crnt_gvn_anmt']);
            header("location: s_diagnostics.php?error_msg=".urlencode("Patient Not Found! Please Add the Patient!"));
        }
        else
        {
            while($search_result=mysql_fetch_array($q))
            ...
            $_SESSION['b_id']=$fet_b_id[0];
            $b_id=$_SESSION['b_id'];//storing bill id in session
            header("location: s_diagnostics.php?error_msg1=".urlencode("Patient Found"));
        }
    }
