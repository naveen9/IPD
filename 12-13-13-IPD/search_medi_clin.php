...
header("location: medical_clienthome.php?error_msg=".urlencode("Please Enter a Searchword!"));
}
else
{
...
if($found==0)
{
    ...
    unset($_SESSION['bed_no']);
    unset($_SESSION['crnt_gvn_anmt']);
header("location: medical_clienthome.php?error_msg=".urlencode("Patient Not Found! Please Add the Patient!"));
}
else
{
...
header("location: medical_clienthome.php?error_msg1=".urlencode("Patient Found"));
}
}
    }
    else
...
if($_POST['find1'])
{
    ...
    $search1=$_POST['search1'];
        ...
        {
            $q=mysql_query("select P.patient_id,P.p_name,P.p_age,P.p_gender,P.p_mob,V.visit_id,V.bed_no,V.ref_dr from patient_info P,visit_register V where V.bed_no='$search1' and V.bed_no!='Discharge patient' and P.patient_id=V.p_id");
            $found=mysql_num_rows($q);
            if($found==0)
            {
               unset($_SESSION['p_id']);
...
//$resVisit=mysql_query("insert into visit_register values('','$current_date','$current_time','$p_id',$proc_status)");
               ...
               // $_SESSION['bed_no']=$fet_v_id[5];
                ...
                $_SESSION['b_id']=$fet_b_id[0];
                $b_id=$_SESSION['b_id'];//storing bill id in session
                header("location: medical_clienthome?error_msg1=".urlencode("Patient Found"));
    }
        }
    }
